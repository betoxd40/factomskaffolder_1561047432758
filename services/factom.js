 ...
 */
const factomize = async (currentModelName, bodyRequest, httpMethod, factomizeNameFK, currentModelId = null,) => {
    try {
        ...
        if(httpUpper === 'POST') {
            ...
            modelFactomizeFKId = await getRelationIdentityId(currentModelId, currentModelName, factomizeNameFK);
        }

        if (!(httpUpper === 'POST' || httpUpper === 'PUT' || httpUpper === 'PATCH' || httpUpper === 'DELETE')) throw new Errors.INVALID_HTTP_METHOD();
        if (!model) throw new Errors.INVALID_MODEL();
            ...
            if(httpUpper === 'PUT') {
                ...
                const currentModelResult = await Database.getConnection().models[currentModelName].findByPk(currentModelId)
                
                // We validate if the fk name is wrong
                if (!(factomizeNameFK in currentModelResult.dataValues)) throw new Errors.INVALID_IDENTITY_MODEL_FK();
                ...
                createEntry(bodyRequest[factomizeNameFK], bodyRequest, factomizedModel, httpUpper);
            } else if (httpUpper === 'DELETE') {
                createEntry(modelFactomizeFKId, { id: currentModelId}, factomizedModel, httpUpper);
            }
    } catch (e) {
        throw e;
    }
...
const getRelationIdentityId = async (currentModelId, currentModelName, factomizeNameFK) => {
    try {
        const modelResult = await Database.getConnection().models[currentModelName].findOne({
...
*/
const createEntry = async (modelFactomizeFKId, bodyRequest, factomizedModel, httpMethod) => {
    try {
        // Get The Model Relation identity related to the current Model 
        const factomizedModelResult = await Database.getConnection().models[factomizedModel].findByPk(modelFactomizeFKId)
        ...
        EntryModel.create(
            chain_id, key_pairs[0].private_key, signerChainId, currentModelData, _id,
        );
